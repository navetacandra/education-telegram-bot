...
class Roboguru {
  ...
  async search(grade_id, subject_id, question) {
      ...
      try {
        ...
        if (!html.includes("application/ld+json")) {
          html = await (
            await fetch(
              u.replace(
                "https://roboguru.ruangguru.com/forum/",
                "https://roboguru.ruangguru.com/question/"
              )
            )
          ).text();
        }
    ...
    let result = (await Promise.all(ld_data)).map((json) => {
      ...
      const { acceptedAnswer, suggestedAnswer } = main;
      ...
      return {
        ...
        answers: [acceptedAnswer ?? [], suggestedAnswer ?? []]
          .flat()
          .map((e) => ({
            text: e.text,
            image: e.image,
